...
- Разобраться с `favicon_debug.log` — файл создаётся рядом с exe при запуске,
  выяснить зачем и убрать если это отладочный вывод
...
#### 🐛 Известные проблемы
- `favicon_debug.log` создаётся рядом с exe при запуске — добавлен в `.gitignore`
